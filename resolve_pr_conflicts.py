...
def parse_conflicts_smart(content):
    ...
    改进：更严格地验证，确保只提取真正的冲突块
    """
    lines = content.split('\n')
    conflicts = []
    depth = 0  # 跟踪嵌套深度
    current_conflict = None
    start_marker_count = 0  # 跟踪当前冲突块中的开始标记数量
    end_marker_count = 0    # 跟踪当前冲突块中的结束标记数量
...
def main():
    ...
    try:
        ...
        # 改进：先检查源分支是否已经包含冲突标记
        print(f"🔍 检查源分支 {pr_info['head_ref']} 是否已包含冲突标记...")
        ...
        if existing_conflicts:
            print(f"✅ 在源分支中发现 {sum(len(s) for s in existing_conflicts.values())} 个冲突标记")
            print("   使用源分支中已有的冲突标记，跳过 git merge")
            conflicts = existing_conflicts
        else:
            # 如果没有冲突标记，执行 git merge 来生成冲突标记
            print(f"🔀 源分支无冲突标记，尝试合并 {pr_info['base_ref']} 到 {pr_info['head_ref']}")
