...
def main():
    ...
    # 解析 PR 信息
    try:
        owner, repo_name, pr_number = parse_pr_url(sys.argv[1])  # 改名为 repo_name 避免冲突
    except Exception as e:
    ...
    try:
        ...
        print("📥 克隆仓库...")
        stdout, stderr, code = run_git_cmd(repo_path, ["clone", repo_url, "."])
        if code != 0:
            print(f"❌ 克隆失败: {stderr}")
            sys.exit(1)
        ...
        # 切换到 PR 分支
        print(f"🔀 切换到 PR 分支: {pr_info['head_ref']}")
        stdout, stderr, code = run_git_cmd(repo_path, ["checkout", pr_info['head_ref']])
        if code != 0:
            print(f"❌ 切换分支失败: {stderr}")
            sys.exit(1)
        
        stdout, stderr, code = run_git_cmd(repo_path, ["pull", "origin", pr_info['head_ref']])
        if code != 0:
            print(f"⚠️  拉取分支失败: {stderr}")
        
        # 获取目标分支
        print(f"📥 获取目标分支: {pr_info['base_ref']}")
        stdout, stderr, code = run_git_cmd(repo_path, ["fetch", "origin", pr_info['base_ref']])
        if code != 0:
        ...
        merge_result = subprocess.run(
            ["git", "merge", f"origin/{pr_info['base_ref']}"],
            cwd=repo_path,
            capture_output=True,
            text=True
        )
        ...
        git_repo = Repo(str(repo_path))  # 使用 git_repo 避免变量名冲突
        ...
        # 推送到远程 - 关键修复：使用 repo_name 而不是 repo
        print("🚀 推送到远程仓库...")
        push_url = f"https://{token}@github.com/{owner}/{repo_name}.git"  # 使用 repo_name
